...
생성 결과는 저장 및 PDF로 다운로드하고, 문제까지 풀 수 있어 학습의 전 과정을 하나의 서비스 안에서 완성할 수 있도록 설계했습니다.`,
    ...
    solution: [
      "pdfjs-dist는 브라우저 환경을 전제로 설계된 라이브러리라 서버에서 그대로 쓸 수 없었습니다. Node.js 환경에 맞게 Worker 옵션을 비활성화하고, 한글 추출이 불안정한 경우 pdf-parse를 병행하는 방식으로 두 라이브러리를 상호보완적으로 활용해 해결했습니다.",
...
개발 과정에서 인증, 파일 스토리지, 비용 최적화까지 실제 서비스 수준의 구현을 목표로 삼았습니다.`,
    challenges: [
      "TTS 생성과 동화 저장을 하나의 흐름으로 묶었더니, TTS가 실패하면 동화 자체가 저장되지 않는 문제가 있었습니다.",
